# 👋 Olá, eu sou Ricardo Nogueira Miranda Filho!

## 📑 Sumário

- 📌 **[Tecnologias & Ferramentas](#-tecnologias--ferramentas)**
- 📦 **[Principais Repositórios](#-principais-repositórios)**
- 💼 **[Experiência Laboral](#-experiência-laboral)**
- 🎓 **[Formação Acadêmica](#-formação-acadêmica)**
- 📚 **[Capacitações](#-capacitações)**
- 🌐 **[Onde me encontrar?](#-onde-me-encontrar)**
- 📝 **[Publicações](#-publicações)**
- 📖 **[Minhas Leituras Recentes](#-minhas-leituras-recentes)**

---

<p align="center">

<!-- GitHub Stats -->
<img src="https://github-readme-stats.vercel.app/api?username=RicardoNMFilho&show_icons=true&theme=tokyonight" width="450"/>

<!-- Top Langs -->
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=RicardoNMFilho&layout=compact&theme=tokyonight" width="350"/>

[![GitHub Streak](https://github-readme-streak-stats.herokuapp.com?user=RicardoNMFilho&hide_total_contributions=true)](https://git.io/streak-stats)

</p>

---

## 🛠 Tecnologias & Ferramentas

<div>
  <!-- Python, Android, PyTorch, TensorFlow, Hugging Face, LangChain -->
  <img alt="Python" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" />
  <img alt="PyTorch" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" />
  <img alt="TensorFlow" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" />
  <img alt="Hugging Face" height="30" src="https://huggingface.co/datasets/huggingface/brand-assets/resolve/main/hf-logo.svg" />

  <br>

  <!-- Android, Java, Kotlin, C, C++ -->
  <img alt="Android" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/android/android-original.svg" />
  <img alt="Java" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" />
  <img alt="Kotlin" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg" />
  <img alt="C" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" />
  <img alt="C++" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" />

  <br>

  <!-- HTML, CSS, JS, TS, Bootstrap, Node -->
  <img alt="HTML" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" />
  <img alt="CSS" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" />
  <img alt="JavaScript" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" />
  <img alt="TypeScript" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" />
  <img alt="Bootstrap" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" />
  <img alt="Node.js" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" />
</div>

---

## 💡 Principais Repositórios

> 🚧 **Em Construção:** Esta seção está sendo atualizada e novos projetos serão adicionados em breve.

---

## 👨‍💻 Experiência Laboral

<details open>
<summary><strong>🎓 Bolsista CAPES — Mai/2025 – Presente</strong> | Universidade Federal do Amazonas (UFAM)</summary>
<br>
Atualmente sou bolsista da CAPES. Desenvolvo pesquisas focadas em técnicas avançadas de Machine Learning para ambientes embarcados, com ênfase em eficiência computacional e energética.

</details>

<br>

<details>
<summary><strong>🔬 SWPERFI RD&I Project — Nov/2022 – Mar/2025</strong> | Universidade Federal do Amazonas (UFAM)</summary>

#### Pesquisador Mestrando (MS II)

Atuei no projeto *"Inteligência Artificial para Análise e Otimização de Desempenho de Software"*. As atividades envolveram modelagem de IA para ambientes com recursos limitados, aplicando técnicas como **quantização** e **destilação de conhecimento** em redes neurais (RNNs, CNNs e LLMs). Também realizei tarefas de **inferência embarcada**, **validação**, **teste de modelos** e **visão computacional** para predição de call drops.

...

#### Pesquisador Graduando e Desenvolvedor

Desenvolvi tarefas de **benchmarking de linguagens de programação** (C, C++, Java, Kotlin, Python, Rust, Golang) para Android, analisando desempenho e uso de bateria em dispositivos diversos. Participei de reuniões com a equipe de bateria do Eldorado Campinas para análise de **logs de Android** e **detecção de anomalias** via aprendizado de máquina.

</details>


---

## 🎓 Formação Acadêmica

### Mestrando em Inteligência Artificial e Ciência de Dados  
**PPGI / IComp / Universidade Federal do Amazonas**  
📅 2025 - Andamento  
📍 Manaus, AM  
Otimização de desempenho de modelos de aprendizado profundo em sistemas embarcados

···

### Bacharel em Ciência da Computação  
**Universidade Federal do Amazonas**  
📅 2021 - 2025  
📍 Manaus, AM  
HarpIA: Uma ferramenta para análise comparativa de frameworks de IA embarcada em Android com foco em eficiência energética

···

### Técnico Integrado em Mecatrônica  
**CEFET-MG / IFAM**  
📅 2019 - 2021  
📍 Belo Horizonte, MG / Manaus, AM  
PCCT Óculos de Detecção de Emoção: Executando Redes Neurais em ESP32


---

## 💼 Capacitações

### DevTITANS - Desenvolvimento, Tecnologia e Inovação em Android e Sistemas Embarcados (2023)

- Projeto do IComp/UFAM em parceria com Motorola e Flextronics, focado na formação de profissionais especializados em Android para Sistemas Embarcados.  
- Capacitação em criação, personalização e manutenção de sistemas Android baseados no Android Open Source Project (AOSP) para diferentes dispositivos de hardware, com conhecimentos avançados em adaptação de código-fonte, integração de serviços e desenvolvimento de tecnologias inovadoras para o mercado.

### Web Academy - Capacitação em Desenvolvimento Web Full Stack (Em Andamento)

- Capacitação intensiva em desenvolvimento web full stack, focado na concepção, projeto, implementação e testes de aplicações para Internet.  
- Formação prática com tecnologias modernas como HTML5, CSS3, JavaScript, Node.js, Typescript, React, Express, MySQL, Docker, GitHub Actions e metodologias ágeis.  
- Inclui também módulos de Cibersegurança, Ciência de Dados, Aprendizado de Máquina e desenvolvimento prático de projeto hands-on.

---

## 📫 Onde me encontrar?

| Plataforma           | Link                                                                                     |
|---------------------|------------------------------------------------------------------------------------------|
| 🔗 LinkedIn          | [Ricardo Nogueira Miranda Filho](https://www.linkedin.com/in/ricardo-nm-filho/)          |
| 🐙 GitHub            | [RicardoNMFilho](https://github.com/RicardoNMFilho)                                     |
| 📧 Email             | [ricardo.filho@icomp.ufam.edu.br](mailto:ricardo.filho@icomp.ufam.edu.br)                |
| 🎓 Lattes            | [Currículo Lattes](http://lattes.cnpq.br/5759583923188184)                              |
| 🆔 ORCID             | [ORCID iD](https://orcid.org/0009-0006-2100-2533)                                       |
| 📸 Instagram         | [@ricardo.mfilho](https://www.instagram.com/ricardo.mfilho/)  

---

## 📚 Publicações

### 📰 Artigos em Journals

- Bessa, João Alfredo, Ricardo Miranda Filho, et al. (May 2025). “Log parsers’ performance on raw logs from Android devices”. *Journal of Internet Services and Applications* 16:1, pp. 105–116. DOI: [10.5753/jisa.2025.5049](https://journals-scholarsportal-org.bbc.org.br/index.php/jisa/article/view/5049).

### 🎤 Anais de Conferências

- Bessa, João Alfredo, Ricardo Filho, et al. (July 2024). “Análise de desempenho de Log Parsers da coleção Logpai em dados brutos de dispositivos Android”. In: *Anais do XXIII Workshop em Desempenho de Sistemas Computacionais e de Comunicação (WPerformance 2024)*. WPerformance 2024. Sociedade Brasileira de Computação - SBC, pp. 37–48. DOI: [10.5753/wperformance.2024.2423](http://dx.doi.org/10.5753/wperformance.2024.2423).

- Filho, Ricardo Miranda et al. (Aug. 2024). “Measuring the Execution Time of Programs from different Android Embedded Programming Languages”. In: *2024 Latin American Computer Conference (CLEI)*. IEEE, pp. 1–9. DOI: [10.1109/clei64178.2024.10700295](http://dx.doi.org/10.1109/clei64178.2024.10700295).

- Matias, Pedro Victor dos Santos, Ricardo Miranda Filho, and Rosiane de Freitas (Nov. 2024). “Understanding the challenges of the call drop prediction problem in IP Multimedia Subsystem Networks”. In: *Anais do XXI Encontro Nacional de Inteligência Artificial e Computacional (ENIAC 2024)*. ENIAC 2024. Sociedade Brasileira de Computação - SBC, pp. 263–274. DOI: [10.5753/eniac.2024.245284](http://dx.doi.org/10.5753/eniac.2024.245284).

- Souza, Girlane et al. (Oct. 2024). “Where Did My Memory Go? An Interactive Visualization Approach to Investigate Memory Consumption on Android Devices”. In: *2024 IEEE Working Conference on Software Visualization (VISSOFT)*. IEEE, pp. 106–110. DOI: [10.1109/vissoft64034.2024.00021](http://dx.doi.org/10.1109/vissoft64034.2024.00021).

- Ventilari, Fernando et al. (Sept. 2023). “Análise comparativa entre linguagens de programação em sistemas embarcados móveis Android”. In: *Proceedings of the XXVII Brazilian Symposium on Programming Languages*. SBLP 2023. ACM, pp. 56–63. DOI: [10.1145/3624309.3624319](http://dx.doi.org/10.1145/3624309.3624319).

---

## ⚡ Minhas Leituras Recentes

> "O mistério da existência humana não consiste apenas em viver,  
> mas em encontrar por que se vive."  
> — Fiódor Dostoiévski, Os Irmãos Karamázov

### Últimos livros lidos:

- **Monte dos Ventos Uivantes** – Emily Brontë

- **O Eterno Marido** – Fiódor Dostoiévski

- **O Beco** – Junji Ito

### Atualmente lendo:

- **Memórias do Subsolo** – Fiódor Dostoiévski

Confira meu perfil no Goodreads para acompanhar todas as minhas leituras:  
[![Goodreads](https://badgen.net/badge/icon/goodreads?icon=goodreads&label=Goodreads)](https://www.goodreads.com/user/show/190425669-ricardo-miranda)

---



<img src="https://media1.tenor.com/m/TRZhUKDNdIEAAAAC/neco-arc-work-neco-arc.gif" width="200">

<div align="center">
  Feito com ❤️ por Ricardo NM Filho
</div>
